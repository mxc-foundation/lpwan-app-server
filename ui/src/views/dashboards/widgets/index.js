...
const TotalUsers = ({ totalUsers }) => {
    return <StatWidget label={i18n.t(`${packageNS}:menu.dashboard.totalUsers.title`)}
        value={totalUsers} formatNum={true}></StatWidget>
}

const TotalOrgs = ({ totalOrganizations }) => {
    return <StatWidget label={i18n.t(`${packageNS}:menu.dashboard.totalOrgs.title`)}
        value={totalOrganizations} formatNum={true}></StatWidget>
}

const TotalGateway = ({ totalGateways }) => {
    return <StatWidget label={i18n.t(`${packageNS}:menu.dashboard.totalGateways.title`)}
        value={totalGateways} formatNum={true}></StatWidget>
}

const TotalDevices = ({ totalDevices }) => {
    return <StatWidget label={i18n.t(`${packageNS}:menu.dashboard.totalDevices.title`)}
        value={totalDevices} formatNum={true}></StatWidget>
}

const TotalApps = ({ totalApps }) => {
    return <StatWidget label={i18n.t(`${packageNS}:menu.dashboard.totalApps.title`)}
        value={totalApps} formatNum={true}></StatWidget>
}

const DataPacketsReceived = ({ packetsReceived }) => {
    return <DataPacketChart data={packetsReceived} color="#10c469"
...
const DataPacketsSent = ({ packetsSent }) => {
    return <DataPacketChart data={packetsSent} color="#71b6f9"
...
const DataPacketsByChannel = ({ packetsByChannel }) => {
    return <DataPacketChart data={packetsByChannel} color="#71b6f9"
        title={i18n.t(`${packageNS}:menu.dashboard.packetsByChannel.title`)}
        labelField="channel" showYAxis={true} />
}

const DataPacketsBySpread = ({ packetsBySpreadFactor }) => {
    return <DataPacketChart data={packetsBySpreadFactor} color="#71b6f9"
...
const adminWidgetCatalog = [
    ...
    {
        type: WIDGET_TYPE_MAP, name: 'packetsBySpreadFactor', label: i18n.t(`${packageNS}:menu.dashboard.dataMap.title`),
...
export { WIDGET_TYPE_GRAPH, WIDGET_TYPE_MAP, WIDGET_TYPE_STAT, adminWidgetCatalog };
