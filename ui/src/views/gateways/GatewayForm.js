...
class GatewayForm extends Component {
  ...
  setKVArrayBoards = () => {
    ...
    this.setState(prevState => {
      ...
      let existingStateBoardsAsArray = this.convertObjToArray(
        existingStateBoards
      );
      ...
      let updatedBoards = propBoards.map(el => {
        let resObj = existingStateBoardsAsArray.find(
          x => x.fpgaID === el.fpgaID
        );
  ...
  onNetworkSelect = v => {
    if (
      !this.state.object.networkServerID ||
      (this.state.object.networkServerID &&
        this.state.object.networkServerID !== v.id)
    ) {
      let object = this.state.object;
  ...
  onGatewayProfileSelect = v => {
    if (
      !this.state.object.gatewayProfileID ||
      (this.state.object.gatewayProfileID &&
        this.state.object.gatewayProfileID !== v.id)
    ) {
      let object = this.state.object;
  ...
  onLocationSourceSelect = v => {
    ...
    if (
      (object.location && !object.location.source) ||
      (object.location &&
        object.location.source &&
        object.location.source !== v.id)
    ) {
      let object = this.state.object;
  ...
  formikFormSchema = () => {
    let fieldsSchema = {
      ...
      description: Yup.string().required(i18n.t(`${packageNS}:tr000431`)),
          ...
          <Col>
            <Formik
              ...
              initialValues={{
                ...
                boards:
                  (object.boards !== undefined &&
                    object.boards.length > 0 &&
                    object.boards) ||
                  []
              }}
              validateOnBlur
                  ...
                  <Form onSubmit={handleSubmit} noValidate>
                    ...
                    {!this.props.update && (
                      <>
                        <Field
                          ...
                          setFieldValue={() =>
                            setFieldValue("gatewayProfileID", "0", false)
                          }
                          inputProps={{
                            clearable: true,
                            cache: false
                        ...
                        {values.networkServerID && (
                          <>
                            <Field
                              ...
                              onBlur={handleBlur}
                              label={i18n.t(`${packageNS}:tr000597`)}
                      ...
                      <Col lg={3} className="text-right">
                        ...
                        >
                          {this.props.submitLabel ||
                            (this.props.update ? "Save Config" : "Create")}
                        </Button>
                        <Button
                          type="submit"
                          color="secondary"
                      ...
                      <TitleBar
                        buttons={
                          <TitleBarButton
                            ...
                            icon={
                              <i className="mdi mdi-crosshairs-gps mr-1 align-middle"></i>
                            }
                            to="#getlocation"
                                          ...
                                          <TitleBar
                                            buttons={
                                              <TitleBarButton
                                                ...
                                                icon={
                                                  <i className="mdi mdi-delete mr-1 align-middle"></i>
                                                }
                                                onClick={() =>
                    ...
                    <div style={{ margin: "20px 0 10px 20px" }}>
                      ...
                      {errors && Object.keys(errors).length ? (
                        ...
                        >
                          Detected {Object.keys(errors).length} errors. Please
                          fix the validation errors shown in each tab before
                          resubmitting.
                        </div>
                      ) : null}
