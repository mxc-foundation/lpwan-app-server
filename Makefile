...
DEMO_HOST_SERVER=$(shell cat lora-app-server.toml | grep demo_host_server=| sed 's/^demo_host_server=//g')
...
ui/build:
	@echo "Building ui"
	@cd ui && printf 'REACT_APP_M2M_SERVER=$(M2M_SERVER)\nREACT_APP_DEMO_USER=$(DEMO_USER)\nREACT_APP_DEMO_USER_PASSWORD=$(DEMO_USER_PASSWORD)\nREACT_APP_DEMO_HOST_SERVER=$(DEMO_HOST_SERVER)'  >> .env.production 
	@cd ui && printf 'REACT_APP_M2M_SERVER=$(M2M_SERVER_DEV)\nREACT_APP_DEMO_USER=$(DEMO_USER)\nREACT_APP_DEMO_USER_PASSWORD=$(DEMO_USER_PASSWORD)\nREACT_APP_DEMO_HOST_SERVER=$(DEMO_HOST_SERVER)' >> .env.development 
	@cd ui && npm run build
	@mv ui/build/* static
