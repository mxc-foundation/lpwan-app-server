...
type Pool interface {
	Get(hostname string, caCert, tlsCert, tlsKey []byte) (nsextra.NetworkServerExtraServiceClient, error)
}

type client struct {
	client     nsextra.NetworkServerExtraServiceClient
...
func (p *pool) Get(hostname string, caCert, tlsCert, tlsKey []byte) (nsextra.NetworkServerExtraServiceClient, error) {
	defer p.Unlock()
	p.Lock()
...
func (p *pool) createClient(hostname string, caCert, tlsCert, tlsKey []byte) (*grpc.ClientConn, nsextra.NetworkServerExtraServiceClient, error) {
	...
	if err != nil {
		return nil, nil, errors.Wrap(err, "dial network-server(extra) api error")
	}

	return nsClient, nsextra.NewNetworkServerExtraServiceClient(nsClient), nil
}
