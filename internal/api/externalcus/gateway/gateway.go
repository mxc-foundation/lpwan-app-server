...
func NewGatewayAPI(validator authcus.Validator) *GatewayAPI {
	return &GatewayAPI{
		validator: validator,
	}
}
...
func storeGateway(ctx context.Context, req *pb.Gateway, defaultGw *storage.Gateway) (err error) {
	...
	err = storage.Transaction(func(tx sqlx.Ext) error {
		...
		if org.MaxGatewayCount != 0 {
			count, err := storage.GetGatewayCount(ctx, tx, storage.GatewayFilters{OrganizationID: req.OrganizationId})
			if err != nil {
				return helpers.ErrToRPCError(err)
			}
