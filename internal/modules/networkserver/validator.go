...
func (v *Validator) ValidateGlobalNetworkServersAccess(ctx context.Context, flag authcus.Flag, orginizationID int64) (bool, error) {
	...
	switch flag {
	case authcus.Create:
		return Service.St.CheckCreateNetworkServersAccess(ctx, u.Username, orginizationID, u.ID)
	case authcus.List:
		return Service.St.CheckListNetworkServersAccess(ctx, u.Username, orginizationID, u.ID)
	default:
...
func (v *Validator) ValidateNetworkServerAccess(ctx context.Context, flag authcus.Flag, networkserverID int64) (bool, error) {
	...
	switch flag {
	case authcus.Read:
		return Service.St.CheckReadNetworkServerAccess(ctx, u.Username, networkserverID, u.ID)
	case authcus.Update, authcus.Delete:
		return Service.St.CheckUpdateDeleteNetworkServerAccess(ctx, u.Username, networkserverID, u.ID)
	default:
