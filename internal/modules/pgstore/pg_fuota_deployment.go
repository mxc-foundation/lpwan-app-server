...
func (ps *pgstore) CreateFUOTADeploymentForDevice(ctx context.Context, fd *store.FUOTADeployment, devEUI lorawan.EUI64) error {
	...
	if fd.State == "" {
		fd.State = store.FUOTADeploymentMulticastCreate
	}
	...
	_, err = ps.db.ExecContext(ctx, `
		...
		now,
		now,
		store.FUOTADeploymentDevicePending,
		"",
	)
	if err != nil {
		return handlePSQLError(Insert, err, "insert error")
...
func (ps *pgstore) GetFUOTADeployment(ctx context.Context, id uuid.UUID, forUpdate bool) (store.FUOTADeployment, error) {
	var fu string
	if forUpdate {
		fu = " for update"
	}
...
func (ps *pgstore) GetPendingFUOTADeployments(ctx context.Context, batchSize int) ([]store.FUOTADeployment, error) {
	var out []store.FUOTADeployment
...
func (ps *pgstore) UpdateFUOTADeployment(ctx context.Context, fd *store.FUOTADeployment) error {
	if err := fd.Validate(); err != nil {
		return errors.Wrap(err, "validate error")
	}
...
func (ps *pgstore) GetFUOTADeploymentCount(ctx context.Context, filters store.FUOTADeploymentFilters) (int, error) {
...
func (ps *pgstore) GetFUOTADeployments(ctx context.Context, filters store.FUOTADeploymentFilters) ([]store.FUOTADeploymentListItem, error) {
	query, args, err := sqlx.BindNamed(sqlx.DOLLAR, `
		select
			distinct fd.id,
			fd.created_at,
	...
	var items []store.FUOTADeploymentListItem
...
// device.
func (ps *pgstore) GetFUOTADeploymentDevice(ctx context.Context, fuotaDeploymentID uuid.UUID, devEUI lorawan.EUI64) (store.FUOTADeploymentDevice, error) {
	var out store.FUOTADeploymentDevice
...
func (ps *pgstore) GetPendingFUOTADeploymentDevice(ctx context.Context, devEUI lorawan.EUI64) (store.FUOTADeploymentDevice, error) {
	var out store.FUOTADeploymentDevice
...
func (ps *pgstore) UpdateFUOTADeploymentDevice(ctx context.Context, fdd *store.FUOTADeploymentDevice) error {
...
// deployment ID.
func (ps *pgstore) GetFUOTADeploymentDevices(ctx context.Context, fuotaDeploymentID uuid.UUID, limit, offset int) ([]store.FUOTADeploymentDeviceListItem, error) {
	var out []store.FUOTADeploymentDeviceListItem
...
func (ps *pgstore) scanFUOTADeployment(row sqlx.ColScanner) (store.FUOTADeployment, error) {
	var fd store.FUOTADeployment
...
func (ps *pgstore) SetFromRemoteMulticastSetup(ctx context.Context, fuotaDevelopmentID, multicastGroupID uuid.UUID) error {
	_, err := ps.db.ExecContext(ctx, `
		...
		store.FUOTADeploymentDevicePending,
		false,
		store.FUOTADeploymentDeviceError,
