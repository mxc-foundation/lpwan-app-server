...
type OTPPgStore struct {
	db *sql.DB
}

func New(db *sql.DB) *OTPPgStore {
	return &OTPPgStore{
		db: db,
	}
}
...
func (st *OTPPgStore) GetTOTPInfo(ctx context.Context, username string) (otp.TOTPInfo, error) {
	totpConfigQuery := `
...
func (st *OTPPgStore) Enable(ctx context.Context, username string) error {
	userID, err := st.getUserID(ctx, username)
	if err != nil {
		return err
	}
...
func (st *OTPPgStore) Delete(ctx context.Context, username string) error {
	userID, err := st.getUserID(ctx, username)
	if err != nil {
		return err
	}
...
func (st *OTPPgStore) StoreNewSecret(ctx context.Context, username, secret string) error {
	tx, err := st.db.BeginTx(ctx, &sql.TxOptions{Isolation: sql.LevelSerializable})
	if err != nil {
		return err
	}
	if err := storeNewSecret(ctx, tx, username, secret); err != nil {
...
func storeNewSecret(ctx context.Context, tx *sql.Tx, username, secret string) error {
	totpConfigQuery := `
...
func (st *OTPPgStore) DeleteRecoveryCode(ctx context.Context, username string, codeID int64) error {
	userID, err := st.getUserID(ctx, username)
	if err != nil {
		return err
	}
...
func (st *OTPPgStore) AddRecoveryCodes(ctx context.Context, username string, codes []string) error {
	tx, err := st.db.BeginTx(ctx, &sql.TxOptions{Isolation: sql.LevelSerializable})
	if err != nil {
		return err
	}
	if err := addRecoveryCodes(ctx, tx, username, codes); err != nil {
		if txErr := tx.Rollback(); txErr != nil {
			ctxlogrus.Extract(ctx).WithError(txErr).Error("couldn't rollback transaction")
		}
...
func addRecoveryCodes(ctx context.Context, tx *sql.Tx, username string, codes []string) error {
	totpConfigQuery := `
...
func (st *OTPPgStore) UpdateLastTimeSlot(ctx context.Context, username string, previousValue, newValue int64) error {
	tx, err := st.db.BeginTx(ctx, &sql.TxOptions{Isolation: sql.LevelSerializable})
	if err != nil {
		return err
	}
	if err := updateLastTimeSlot(ctx, tx, username, previousValue, newValue); err != nil {
		if txErr := tx.Rollback(); txErr != nil {
			ctxlogrus.Extract(ctx).WithError(txErr).Error("couldn't rollback transaction")
		}
...
func updateLastTimeSlot(ctx context.Context, tx *sql.Tx, username string, previousValue, newValue int64) error {
	totpConfigQuery := `
		SELECT u.id, utc.last_time_slot
		FROM "user" u LEFT JOIN totp_configuration utc ON (u.id = utc.user_id)
		WHERE u.username = $1
